...
import {
  FilesetResolver as TextFilesetResolver,
  TextEmbedder,
...
export const ImageEmbed: React.FC = () => {
  ...
  const onStart = async () => {
    const response2 = await axios.get<{ imageId: number }[]>(
  ...
  const onClick = async () => {
    const response = await axios.get<
      { imageId: number; imageEmbedding: string }[]
    >(`/api/collections/imageEmbedding`);
    const target = response.data.find((v) => v.imageId.toString() === value);
    const result = response.data
      .map((v) => {
        const similar = ImageEmbedder.cosineSimilarity(
          {
            floatEmbedding: JSON.parse(target.imageEmbedding) as number[],
            headName: "feature",
            headIndex: 0,
          },
          {
            floatEmbedding: JSON.parse(v.imageEmbedding) as number[],
            headName: "feature",
            headIndex: 0,
          }
        );
  ...
  const onTextClick = async () => {
    const response = await axios.get<
      { imageId: number; textEmbedding: string }[]
    >(`/api/collections/textEmbedding`);
    const target = response.data.find((v) => v.imageId.toString() === value);
    ...
    const result = response.data
      .map((v) => {
        ...
        const similar = TextEmbedder.cosineSimilarity(
          {
            floatEmbedding: JSON.parse(target.textEmbedding) as number[],
            headName: "feature",
            headIndex: 0,
          },
          {
            floatEmbedding: JSON.parse(v.textEmbedding) as number[],
            headName: "feature",
            headIndex: 0,
          }
        );
  ...
  const onColorClick = async () => {
    const rgb = color.split(",").map((v) => parseInt(v)) as [
      number,
      number,
      number
    ];
    ...
    response.data.forEach((color) => {
      ...
      const lab2 = color.color
        .split(",")
        .map((v) => parseFloat(v.replace(/[()]/g, ""))) as [
        number,
        number,
        number
      ];
    ...
    const uniq = (arr: { imageId: number }[], track = new Set()) =>
      arr.filter(({ imageId }) =>
        track.has(imageId) ? false : track.add(imageId)
      );
  ...
  return (
    <div>
      ...
      <button onClick={onTextStart} disabled={disabled}>
        전체 텍스트 임베딩 시작
      </button>
      ...
      <button onClick={onTextClick} disabled={disabled}>
        텍스트 유사도 조회
      </button>
      <br />
      ...
      <div>
        {result.slice(0, 20).map((v) => (
          <div>
            <img
              src={`/api/admin/images/${v.imageId}/thumb`}
              alt=""
              width={128}
              height={128}
            />
