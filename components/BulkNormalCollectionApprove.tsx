...
export const getActionElementCss = (
  resourceId: string,
  actionName: string,
  suffix: string
) => `${resourceId}-${actionName}-${suffix}`;

const BulkNormalCollectionApprove: React.FC<ActionProps & AddNoticeProps> = (
  props
) => {
        ...
        >
          ...
          {translateButton(
            records.length > 1
              ...
              : "confirmNormalBulkCollectionApprove",
            resource.id,
            { count: records.length }
          )}
        </Button>
...
export default withNotice(BulkNormalCollectionApprove) as React.FC;
